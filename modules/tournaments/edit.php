...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($gameName) || empty($startDate) || empty($endDate)) {
        ...
        try {
            $stmt = $pdo->prepare("
                UPDATE tournaments 
                SET title = ?, game_name = ?, description = ?, prize_pool = ?, registration_fee = ?, 
            ...
            if ($stmt->execute([$title, $gameName, $description, $prizePool, $registrationFee, $startDate, $endDate, $maxParticipantsValue, $status, $tournamentId])) {
                flashMessage('Tournament updated successfully!', 'success');
                header('Location: dashboard.php?page=tournaments');
                exit;
            } else {
    ...
    <div class="col-md-10 offset-md-1">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"><i class="bi bi-pencil me-2"></i>Edit Tournament</h3>
            </div>
            <div class="card-body">
                <?php if ($errorMessage): ?>
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="3"
                                  placeholder="Tournament details and rules..."><?= htmlspecialchars($tournament['description'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" id="prize_pool" name="prize_pool" step="0.01" min="0"
                                   value="<?= htmlspecialchars($tournament['prize_pool'] ?? '0') ?>"
                                   placeholder="0.00">
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" id="registration_fee" name="registration_fee" step="0.01" min="0"
                                   value="<?= htmlspecialchars($tournament['registration_fee'] ?? '0') ?>"
                                   placeholder="0.00">
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" id="max_participants" name="max_participants" min="1"
                                   value="<?= htmlspecialchars($tournament['max_participants'] ?? '') ?>"
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="status" name="status" required>
                            <?php foreach ($statusOptions as $option): ?>
                                <option value="<?= $option ?>" <?= ($tournament['status'] ?? 'upcoming') === $option ? 'selected' : '' ?>>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-circle me-1"></i> Update Tournament
                        </button>
